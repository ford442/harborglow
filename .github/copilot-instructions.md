...
- `src/App.tsx` owns the top-level screen flow (`menu` -> `loading` -> `game` / `training`), resumes `audioRuntime` on first user gesture, lazy-loads `MainScene`, and mounts it inside `@react-three/fiber` `Canvas` + `@react-three/rapier` `Physics`. `HUD` and `TrainingHUD` stay outside the canvas as DOM overlays.
...
- Audio/visual show logic is singleton-based. `src/systems/musicSystem.ts` owns per-ship band metadata, lyrics, instruments/bus effects on the sim-clocked beat `transport` (`src/systems/audio/`); related systems such as `lightingSystem` and `audioVisualSync` react to the same gameplay milestones.
