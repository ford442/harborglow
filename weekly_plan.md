...
## Today's focus
**2026-08-03 — Orphan scene consolidation (#162).** Archive parallel water stacks (`FFTOcean`, `PBRWater`, `InteractiveWater`), unused `ExperimentalTech` renderer, and `MonitorMinimalExample` out of `src/scenes/`. `Water.tsx` remains the sole water authority (Gerstner + `WaveSystem` + `qualityPreset` tiers). Refresh `AGENTS.md`, `docs/STORE.md`, and backlog notes to match `main`: domain-slice store, hero GLBs in `public/models/`, unified Harbor Credits wallet, **338/338** Vitest tests green (`src/test/setup.ts` tone stub is on `main`).
...
-->
- [x] ~~**P0 — tone stub regression:**~~ **RESOLVED on `main`** — `src/test/setup.ts` + `toneMock.ts` + `test.setupFiles` present; `npm test` = **25/25 suites, 338/338 tests** (2026-08-03).
- [ ] **P1 — CI job ordering hardening (issue #161):** parallel merge gates in `.github/workflows/ci.yml` so a red test job does not skip smoke/build reporting.
- [ ] **ISSUE BACKLOG #158–#165** (filed 2026-07-30 after #142–#149 closed): **#159** WASM DSP stub; **#160** bootstrap lifecycle; **#161** CI parallelize; **#163** hero GLB socket maps; **#164** multi-crane NPC; **#165** WebGPU TSL + FFT ocean epic. **#145 leftover** (`store/types.ts` duplicate) — **resolved** (file deleted; `gameStoreTypes.ts` canonical). **#162** orphan scene consolidation — **in progress this run**.
