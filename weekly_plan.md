...
## Today's focus
**2026-07-12 — New Idea mode.** Foundation is solid: fresh `npm install` + `npm run typecheck` = **0 errors**, `vitest run` = **200/200** (up from 172 — new `wildlifeProfiles.test.ts`, 28 tests). The CI `tsc`/test gate (`da00fe9`) held again — first run in five weeks with no masked build break to catch. Not Fix First.

The sole open Idea — **Wildlife & Scenic Coastal Ecosystem** — is now **feature-complete through Phase 4**. kimi-cli ran last week's dispatch far past the Phase-1 slice: `.swarm-state.md` documents 4 iterations landing (P1) store fields + `wildlifeProfiles.ts` + `ambientMarineLifeSystem.ts` + `AmbientMarineLife.tsx` (Moon Jelly / fish schools / plankton, instanced, gated on `enableMarineLife`); (P2) `SeaBirds.tsx` + gray whale; (P3) camera-frustum spawning + seasonal color tints + `getLightAttractionMultiplier()`; (P4) kelp beds + crane-spreader disturbance hook + distance LOD. Leva "Marine Life" + "Wildlife / Birds" folders (toggle, density slider, season) are wired in `MainSceneHelpers.tsx`. A GLSL screen-space god-rays `ShaderPass` (`src/shaders/GodRaysShader.ts`) also landed via PR #110 — this **supersedes** the staged WGSL compute shader (now orphaned). Ideas list is therefore **exhausted** → New Idea mode.

Today's focus (New Idea, picked): **Music-reactive wildlife choreography + "Bioluminescent Finale."** Two freshly-completed pillars — the ambient marine-life ecosystem and per-band light-shows/cutaways — currently only couple at the coarse `getLightAttractionMultiplier()` density level. The wildlife spec's own core principle ("Music-reactive where it makes sense — bioluminescence pulsing to the beat, fish schooling tighter during intense light shows") is unmet. Today wires beat-level reactivity: plankton/jelly emissive pulses to the Tone.js Transport beat, fish schools tighten cohesion during a light show, and the ship's upgrade **cutaway** triggers a scripted "wildlife gathers" moment (bioluminescent bloom converging on the finished ship). kimi-cli main event, scoped to the wildlife + audio-sync surface. Copilot issue is fully decoupled: a crane-to-ship attachment **feel pass** (magnetic snap guidance + lock feedback + sway settling) — touches `Crane.tsx`/`attachmentSystem.ts`/`swaySystem.ts`/`craneSoundSystem.ts`, zero overlap with wildlife/audio files.
...
- [ ] **New Idea (2026-07-12, appended — partially seeded via today's Copilot issue):** Crane-to-ship attachment **feel pass** — the #1 stated focus area but untouched since 2026-04-19 while polish went to visual/audio/wildlife. Magnetic snap guidance as the spreader nears an attachment point, distinct lock feedback (audio + brief camera/haptic punch), sway settling after bind. Touches `Crane.tsx`/`attachmentSystem.ts`/`swaySystem.ts`/`craneSoundSystem.ts`. Scope: full-day. Today's Copilot issue B kicks off the first pass.
...
Date: 2026-07-12
Mode: New Idea (foundation solid — Ideas list exhausted; second consecutive non-Fix-First run)
Focus: **Music-reactive wildlife choreography + "Bioluminescent Finale"** — wire beat-level reactivity between the now-complete ambient marine ecosystem and the per-band light-shows/cutaways (plankton/jelly emissive pulses to Tone.js Transport beat, fish-school cohesion tightens during a show, upgrade cutaway triggers a scripted wildlife-gathering bioluminescent bloom). Fulfills the wildlife spec's unmet "music-reactive" core principle. kimi-cli main event on the wildlife + audio-sync surface. Decoupled Copilot issue: crane-to-ship attachment feel pass (`Crane.tsx`/`attachmentSystem.ts`/`swaySystem.ts`/`craneSoundSystem.ts`).
Reconciliation: **flagship Wildlife Idea closed — all 4 phases** (kimi-cli overshot the scoped Phase-1 slice through Phase 4 in one swarm run; codespace `c6d4ad3`, `.swarm-state.md` = 4 iterations) → Done + Idea [x]. GLSL god-rays ShaderPass landed via PR #110 (`31f2841`) → Done; **supersedes** the staged WGSL compute shader (backlog item flipped from "wire it" to "delete/keep the orphan"). Ideas list now empty → New Idea mode; proposed 3, picked music-reactive wildlife, appended the other 2 (new vessel class end-to-end; crane attachment feel pass) to Ideas. Build verified green: fresh `npm install` → `typecheck` 0 errors, `vitest` 200/200 — CI gate held, no masked break for a second week.
Carried backlog: light-show stub redirects (`lngShow.ts`/`tankerShow.ts`) still present; root codemods `fix_glb_loader.cjs` ×2 + re-committed `.swarm-state.md` (21 KB — recommend `.gitignore`); orphaned `god-rays-compute.wgsl` (superseded by GLSL — delete/keep decision); GLB loader stub unwired; `createRenderer.ts:45` lint error; **SECURITY credential rotation UNRESOLVED a 5th week** (rotation is a 2-min action, blocks nothing — Noah please do it).
Note: no `recent_chats`/`conversation_search` tools in this environment again (5th run) — Step 1 cross-session sweep skipped; reconciliation from repo state + GitHub MCP (0 open issues, 0 open PRs). Thin cross-session context is a real gap, not hidden.
Outcome: weekly_plan.md updated (flagship Idea closed all-phases, god-rays Done, 2 new Ideas appended, Backlog reconciled, Today's focus + Last run refreshed). Dispatch below: kimi-cli music-reactive wildlife + decoupled crane-attachment Copilot issue + 3 chat prompts + Copilot handoff + Jules template + Claude Code whole-stack + 2 review prompts.
