...
## Today's focus
**2026-08-09 — Real audio-driven beat detection (User Idea, GitHub #181).** Replace the `Tone.FFT` + hardcoded `bpm=128` onset estimator in `audioVisualSync.ts` with a real Web Audio `AnalyserNode` (fftSize 2048) tapped on `Tone.getContext().rawContext` master bus, reading `getByteFrequencyData()` into a pre-allocated `Uint8Array` each `useFrame` (zero-alloc). Compute measured `bass`/`mid`/`treble` band means + a rolling-average (1.5×) onset detector, gated by the existing BPM clock to suppress non-beat frames. Wire the live band values into `TelemetryGraph.tsx` and use `wasmDSP.audioRms` on the WASM path. This is **#1 in Noah's own recommended order of attack** (issue #184 body): pure TypeScript, no new deps, and it **unblocks #179** (TSL beat-reactive shaders, which need live FFT data). Foundation verified green this run: typecheck 0 err, **351/351** Vitest, `vite build` ✓.
...
-->
> **The live idea pool now lives in GitHub Issues #178–#184** (filed by Noah 2026-08-07), not this section. The in-file Ideas list below is fully exhausted (all `[x]`). Noah's own recommended order of attack (issue #184 body): **1) #181** AnalyserNode beat detection → **2) #179** TSL light-show shaders → **3) #180** GLB models → **4) #184** WASM DSP expansion → **5) #178** FFT ocean → **6) #182** SSR → **7) #183** multiplayer.
- [ ] **#181 AnalyserNode FFT beat detection** — `[in progress — 2026-08-09]` (today's kimi-cli focus). Pure TS, unblocks #179.
- [ ] **#179 TSL NodeMaterial beat-reactive light-show shaders** — blocked on #181 (needs live FFT). Next after today.
- [ ] **#180 GLB models for 7 remaining vessel types** — independent; today's decoupled Copilot issue B.
- [ ] **#184 WASM DSP procedural-audio expansion** (additive synth + IR convolution) — C++ foundation, unblocks #178. Needs Emscripten in CI.
- [ ] **#178 WebGPU compute-shader FFT ocean** (Phillips/WGSL) — renderer-level, depends on #184.
- [ ] **#182 SSR post-processing pass** — independent, WebGL2-only path.
- [ ] **#183 Multiplayer spectator (WebRTC)** — largest scope; do last, after store shape stabilizes.
...
- [ ] **NEW ISSUE POOL #178–#184** (filed by Noah 2026-08-07 — the live idea backlog, see Ideas section for the ordered attack plan): **#178** WebGPU FFT ocean; **#179** TSL beat-reactive shaders (blocked on #181); **#180** 7 remaining GLB models (today's Copilot issue B); **#181** AnalyserNode beat detection (today's kimi-cli focus); **#182** SSR pass; **#183** WebRTC multiplayer; **#184** WASM DSP procedural-audio expansion.
...
Date: 2026-08-09
Mode: **USER IDEA** — foundation is green (typecheck 0, 351/351 tests, build ✓), and Noah filed a fresh, dependency-ordered idea backlog as GitHub Issues #178–#184 on 2026-08-07. Picked **#181 (AnalyserNode FFT beat detection)** — #1 in Noah's own order of attack: pure TS, no new deps, unblocks #179.
Focus: **Real audio-driven beat detection.** kimi-cli main event = replace `audioVisualSync.ts`'s `Tone.FFT` + hardcoded `bpm=128` onset estimator with a raw `AnalyserNode` (fftSize 2048, `getByteFrequencyData` into a pre-allocated `Uint8Array`, zero-alloc per frame) tapped on `Tone.getContext().rawContext`; measured bass/mid/treble means + 1.5× rolling-average onset detector gated by the BPM clock; live band values into `TelemetryGraph.tsx`; `wasmDSP.audioRms` on the WASM path. Decoupled Copilot issue B = **#180** (author + integrate GLB models for the 7 remaining vessel types — `src/ships/*`, `public/models/*`, `ships.json`, `scripts/generate-ship-glb.mjs`; zero file overlap with the audio work). Claude Code task E = Vite manual-chunk / bundle-composition hygiene audit (`vendor-3d`/`vendor-audio` integrity, no React duplication across chunks). Jules F = integrate kimi-cli's #181 output into a reviewable PR.
Reconciliation: **Entire #158–#165 backlog landed & closed** (merged 08-02→08-03, PRs #168–#177) → moved to Done. Backlog items resolved this run: tone-stub regression (durable, guard #168), CI job-ordering (#161), repo-root debris (untracked), god-rays WGSL (relocated to `docs/reference/`), light-show stub redirects (gone), `store/types.ts` duplicate (gone). New live idea pool = GitHub #178–#184; in-file Ideas section noted as exhausted with the ordered attack plan recorded.
Carried backlog: **SECURITY credential rotation UNRESOLVED — now a 9th week** (public-repo git history still contains the old SFTP password + deploy token; rotation is a 2-minute Noah action, blocks nothing, please do it); unreferenced scene entry points still in `src/scenes/` (`LightShow.tsx`, `UpgradeCelebration.tsx`, `ParticleSystem.tsx`, `MonitorSystem.tsx`) — archive candidates.
Note: `recent_chats`/`conversation_search` tools absent again (9th run) — Step 1 cross-session sweep skipped; reconciliation done from clean-`npm ci` local gate runs + GitHub Issues/PR API (7 open issues #178–#184, 0 open PRs). Thin cross-session context is a real gap, not hidden.
Outcome: weekly_plan.md updated (Today's focus → User Idea #181 with full technical framing; #158–#165 batch + hygiene wins moved to Done; backlog re-baselined to #178–#184; Ideas section annotated with GitHub-issue idea pool + ordered attack plan, #181 marked in progress; security item carried to week 9). Dispatch below: kimi-cli #181 beat-detection swarm + #180 GLB-model Copilot issue + 3 chat prompts + Copilot handoff + Jules integration template + Claude Code bundle-chunk hygiene + 2 review prompts.
