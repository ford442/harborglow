...
- [in progress — 2026-06-15] Per-band light-show choreography (`claude_contributions.md` §1): give each of the 11 ship-type bands a hand-felt light-cue schedule (`LightCue[]` — beat/pattern/color/intensity) instead of one shared beat-intensity curve. Engine hook in `lightingSystem.ts` + new `src/systems/lightShows/`; start with LNG (cyan breathing) and Oil Tanker (amber/red snaps), expand to remaining 9 ships in follow-up sessions.
  Slice landed 2026-06-15: `LightCue`/`LightPattern` types + `src/systems/lightShows/{types,lngShow,tankerShow,index}.ts`; `lightingSystem.ts` now resolves a per-band cue (looping every 32 beats, matching the cinematic's climax-at-24/hide-at-32 cycle) and dispatches `breathe`/`sweep`/`strobe`/`snap`/`blackout` patterns through `getLightIntensity`/`getEmissiveColor`, falling back to the old generic curve for the other 9 ships. Vitest smoke tests added (`lightingSystem.test.ts`, 100/100 passing). Remaining 9 ships still follow-up work.
...
Outcome: Today's-focus slice implemented 2026-06-15 — `LightCue[]` type, `src/systems/lightShows/` (LNG + Oil Tanker tables), and a cue-dispatch hook in `lightingSystem.ts` with generic-curve fallback. Build, lint, and tests (100/100) all green. Remaining 9 ships carried to follow-up sessions.
